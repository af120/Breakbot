...
async function generateToken(userId, role, secret) {
  const enc = new TextEncoder();
  const header = { alg: "HS256", typ: "JWT" };
  const payload = { userId, role, exp: Math.floor(Date.now() / 1e3) + 24 * 60 * 60 };
  const base64UrlEncode = /* @__PURE__ */ __name((obj) => btoa(JSON.stringify(obj)).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, ""), "base64UrlEncode");
  ...
  const sigBase64Url = btoa(String.fromCharCode(...new Uint8Array(signature))).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
...
var requireRole = /* @__PURE__ */ __name((...roles) => {
  return async (c, next) => {
...
__name(generateBookingRef, "generateBookingRef");
async function createAuditLog(db, action, userId, userName, recordType, recordId, oldValue, newValue, reason = "") {
  await db.prepare(`
...
app.post("/users", requireRole("admin"), async (c) => {
  ...
  await c.env.DB.prepare("INSERT INTO users (id, username, password_hash, role, full_name, is_active, created_at) VALUES (?, ?, ?, ?, ?, 1, ?)").bind(id, username, hash, role, fullName, (/* @__PURE__ */ new Date()).toISOString()).run();
...
app2.post("/", requireRole("admin", "manager"), async (c) => {
  const { name, description, duration_minutes, is_active, is_public } = await c.req.json();
  const id = generateId();
  await c.env.DB.prepare("INSERT INTO services (id, name, description, duration_minutes, is_active, is_public, display_order, created_at) VALUES (?, ?, ?, ?, ?, ?, 0, ?)").bind(id, name, description, duration_minutes, is_active ? 1 : 0, is_public ? 1 : 0, (/* @__PURE__ */ new Date()).toISOString()).run();
  return c.json({ success: true, data: { id } });
});
app2.put("/:id", requireRole("admin", "manager"), async (c) => {
  const { name, description, duration_minutes, is_active, is_public } = await c.req.json();
  await c.env.DB.prepare("UPDATE services SET name = ?, description = ?, duration_minutes = ?, is_active = ?, is_public = ? WHERE id = ?").bind(name, description, duration_minutes, is_active ? 1 : 0, is_public ? 1 : 0, c.req.param("id")).run();
  return c.json({ success: true });
});
app2.delete("/:id", requireRole("admin"), async (c) => {
  await c.env.DB.prepare("UPDATE services SET is_deleted = 1 WHERE id = ?").bind(c.req.param("id")).run();
  return c.json({ success: true });
});
app2.put("/:id/order", requireRole("admin", "manager"), async (c) => {
  const { order } = await c.req.json();
  await c.env.DB.prepare("UPDATE services SET display_order = ? WHERE id = ?").bind(order, c.req.param("id")).run();
  return c.json({ success: true });
});
app2.get("/:id/prices", async (c) => {
  const { results } = await c.env.DB.prepare("SELECT * FROM service_prices WHERE service_id = ?").bind(c.req.param("id")).all();
  return c.json({ success: true, data: results });
});
app2.put("/:id/prices", requireRole("admin", "manager"), async (c) => {
  const { prices } = await c.req.json();
  const id = c.req.param("id");
  await c.env.DB.prepare("DELETE FROM service_prices WHERE service_id = ?").bind(id).run();
  for (const price of prices) {
    await c.env.DB.prepare("INSERT INTO service_prices (id, service_id, vehicle_type, price_iqd) VALUES (?, ?, ?, ?)").bind(generateId(), id, price.vehicle_type, price.price_iqd).run();
  }
  return c.json({ success: true });
});
var services_default = app2;
...
app3.get("/duplicates/:phone", async (c) => {
...
app3.post("/", async (c) => {
  ...
  const id = generateId();
  await c.env.DB.prepare("INSERT INTO customers (id, name, phone, email, notes, created_at) VALUES (?, ?, ?, ?, ?, ?)").bind(id, name, phone, email, notes, (/* @__PURE__ */ new Date()).toISOString()).run();
  return c.json({ success: true, data: { id } });
});
app3.put("/:id", async (c) => {
  const { name, phone, email, notes } = await c.req.json();
  await c.env.DB.prepare("UPDATE customers SET name = ?, phone = ?, email = ?, notes = ? WHERE id = ?").bind(name, phone, email, notes, c.req.param("id")).run();
  return c.json({ success: true });
});
app3.get("/:id/vehicles", async (c) => {
  const { results } = await c.env.DB.prepare("SELECT * FROM vehicles WHERE customer_id = ?").bind(c.req.param("id")).all();
  return c.json({ success: true, data: results });
});
app3.get("/:id/appointments", async (c) => {
...
var customers_default = app3;

// worker/routes/vehicles.ts
var app4 = new Hono2();
app4.get("/", async (c) => {
...
app4.post("/", async (c) => {
  ...
  const id = generateId();
  await c.env.DB.prepare("INSERT INTO vehicles (id, customer_id, make, model, year, color, license_plate, vehicle_type, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(id, customer_id, make, model, year, color, license_plate, vehicle_type, (/* @__PURE__ */ new Date()).toISOString()).run();
  return c.json({ success: true, data: { id } });
});
app4.put("/:id", async (c) => {
  const { make, model, year, color, license_plate, vehicle_type } = await c.req.json();
  await c.env.DB.prepare("UPDATE vehicles SET make = ?, model = ?, year = ?, color = ?, license_plate = ?, vehicle_type = ? WHERE id = ?").bind(make, model, year, color, license_plate, vehicle_type, c.req.param("id")).run();
  return c.json({ success: true });
});
var vehicles_default = app4;

// worker/routes/appointments.ts
var app5 = new Hono2();
app5.get("/", async (c) => {
  const { results } = await c.env.DB.prepare("SELECT * FROM appointments ORDER BY scheduled_for DESC LIMIT 100").all();
  return c.json({ success: true, data: results });
});
app5.get("/today", async (c) => {
  const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
  const { results } = await c.env.DB.prepare("SELECT * FROM appointments WHERE date(scheduled_for) = ?").bind(today).all();
  return c.json({ success: true, data: results });
});
app5.get("/queue", async (c) => {
...
app5.get("/dashboard-stats", async (c) => {
  const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
  const db = c.env.DB;
...
app5.post("/", async (c) => {
  ...
  await c.env.DB.prepare(`INSERT INTO appointments (id, booking_ref, customer_id, vehicle_id, service_id, status, scheduled_for, total_price, source, notes, created_at) 
                          VALUES (?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?, ?)`).bind(id, ref, customer_id, vehicle_id, service_id, scheduled_for, total_price, source, notes, (/* @__PURE__ */ new Date()).toISOString()).run();
  return c.json({ success: true, data: { id, ref } });
});
app5.put("/:id", async (c) => {
  const { scheduled_for, notes, total_price } = await c.req.json();
  await c.env.DB.prepare("UPDATE appointments SET scheduled_for = ?, notes = ?, total_price = ? WHERE id = ?").bind(scheduled_for, notes, total_price, c.req.param("id")).run();
  return c.json({ success: true });
});
app5.put("/:id/status", async (c) => {
  const { status, reason } = await c.req.json();
  const id = c.req.param("id");
  const user = c.get("user");
  const app19 = await c.env.DB.prepare("SELECT status FROM appointments WHERE id = ?").bind(id).first();
  if (!app19) return c.json({ success: false, error: "Not found" }, 404);
  await c.env.DB.prepare("UPDATE appointments SET status = ? WHERE id = ?").bind(status, id).run();
  await createAuditLog(c.env.DB, "update_status", user.userId, "System", "appointments", id, app19.status, status, reason);
  return c.json({ success: true });
});
app5.put("/:id/confirm", async (c) => {
  await c.env.DB.prepare('UPDATE appointments SET status = "confirmed" WHERE id = ?').bind(c.req.param("id")).run();
  return c.json({ success: true });
});
app5.put("/:id/assign-bay", async (c) => {
...
app5.put("/:id/assign-employee", async (c) => {
...
var appointments_default = app5;

// worker/routes/public.ts
var app6 = new Hono2();
app6.post("/book", async (c) => {
  ...
  if (!customer_id) {
    ...
    await db.prepare("INSERT INTO customers (id, name, phone, created_at) VALUES (?, ?, ?, ?)").bind(customer_id, name, phone, (/* @__PURE__ */ new Date()).toISOString()).run();
  }
  let vehicle_id = generateId();
  await db.prepare("INSERT INTO vehicles (id, customer_id, type, brand, model, plate_number, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(vehicle_id, customer_id, vehicle_type, make || "", model || "", plate || "", (/* @__PURE__ */ new Date()).toISOString()).run();
  ...
  await db.prepare(`INSERT INTO appointments (id, booking_ref, customer_id, vehicle_id, status, requested_date, requested_time, estimated_total, source, created_at) 
                          VALUES (?, ?, ?, ?, 'pending', ?, ?, ?, 'web', ?)`).bind(id, ref, customer_id, vehicle_id, scheduled_for.split("T")[0], scheduled_for.split("T")[1] || "10:00", price, (/* @__PURE__ */ new Date()).toISOString()).run();
  await db.prepare(`INSERT INTO appointment_services (id, appointment_id, service_id, vehicle_type, price_snapshot) VALUES (?, ?, ?, ?, ?)`).bind(generateId(), id, service_id, vehicle_type, price).run();
  return c.json({ success: true, data: { ref } });
});
app6.get("/booking-status", async (c) => {
...
app6.get("/settings", async (c) => {
  const { results } = await c.env.DB.prepare("SELECT * FROM settings").all();
  const settings = results.reduce((acc, r) => ({ ...acc, [r.key]: r.value }), {});
  return c.json({ success: true, data: settings });
});
app6.get("/gallery", async (c) => {
  return c.json({ success: true, data: [] });
});
app6.get("/testimonials", async (c) => {
  return c.json({ success: true, data: [] });
});
app6.get("/content", async (c) => {
  return c.json({ success: true, data: [] });
});
app6.get("/availability", async (c) => {
  return c.json({ success: true, data: [] });
});
var public_default = app6;

// worker/routes/bays.ts
var app7 = new Hono2();
app7.get("/", async (c) => {
...
app7.put("/:id/status", async (c) => {
  const { status } = await c.req.json();
  await c.env.DB.prepare("UPDATE washing_bays SET current_status = ? WHERE id = ?").bind(status, c.req.param("id")).run();
  return c.json({ success: true });
});
app7.post("/:id/blocks", async (c) => {
  return c.json({ success: true });
});
app7.delete("/blocks/:id", async (c) => {
  return c.json({ success: true });
});
var bays_default = app7;

// worker/routes/employees.ts
var app8 = new Hono2();
app8.get("/", async (c) => {
...
app8.get("/:id/assignments", async (c) => {
  const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
...
app9.post("/", async (c) => {
  ...
  const id = generateId();
  await c.env.DB.prepare("INSERT INTO payments (id, appointment_id, amount, method, reference_number, notes, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(id, appointment_id, amount, method, reference, notes, (/* @__PURE__ */ new Date()).toISOString()).run();
  return c.json({ success: true, data: { id } });
});
app9.get("/:appointmentId", async (c) => {
  const { results } = await c.env.DB.prepare("SELECT * FROM payments WHERE appointment_id = ?").bind(c.req.param("appointmentId")).all();
  return c.json({ success: true, data: results });
});
app9.put("/:id", requireRole("admin", "manager"), async (c) => {
...
app10.post("/", async (c) => {
  ...
  const id = generateId();
  await c.env.DB.prepare("INSERT INTO expenses (id, amount, category, description, date, created_at) VALUES (?, ?, ?, ?, ?, ?)").bind(id, amount, category, description, date, (/* @__PURE__ */ new Date()).toISOString()).run();
  return c.json({ success: true, data: { id } });
});
app10.put("/:id", async (c) => {
  const { amount, category, description, date } = await c.req.json();
  await c.env.DB.prepare("UPDATE expenses SET amount = ?, category = ?, description = ?, date = ? WHERE id = ?").bind(amount, category, description, date, c.req.param("id")).run();
  return c.json({ success: true });
});
...
var expenses_default = app10;

// worker/routes/gallery.ts
var app11 = new Hono2();
app11.get("/", async (c) => {
...
app11.post("/", async (c) => {
  ...
  const id = generateId();
  await c.env.DB.prepare("INSERT INTO gallery (id, image_url, title, description, is_public, display_order) VALUES (?, ?, ?, ?, ?, 0)").bind(id, url, title, description, is_public ? 1 : 0).run();
  return c.json({ success: true, data: { id } });
});
app11.post("/upload", async (c) => {
  return c.json({ success: true, data: { url: "https://example.com/img.jpg" } });
});
app11.put("/:id/order", async (c) => {
  const { order } = await c.req.json();
...
var gallery_default = app11;

// worker/routes/testimonials.ts
var app12 = new Hono2();
app12.get("/", async (c) => {
...
app12.post("/", async (c) => {
  ...
  const id = generateId();
  await c.env.DB.prepare("INSERT INTO testimonials (id, customer_name, content, rating, is_published) VALUES (?, ?, ?, ?, ?)").bind(id, customer_name, content, rating, is_published ? 1 : 0).run();
  return c.json({ success: true, data: { id } });
});
app12.put("/:id", async (c) => {
  const { customer_name, content, rating, is_published } = await c.req.json();
  await c.env.DB.prepare("UPDATE testimonials SET customer_name = ?, content = ?, rating = ?, is_published = ? WHERE id = ?").bind(customer_name, content, rating, is_published ? 1 : 0, c.req.param("id")).run();
  return c.json({ success: true });
});
...
var testimonials_default = app12;

// worker/routes/reports.ts
var app13 = new Hono2();
app13.get("/income", async (c) => {
...
app13.get("/summary", async (c) => {
  return c.json({ success: true, data: { gross: 1e3, expenses: 200, net: 800 } });
});
app13.get("/services", async (c) => {
  return c.json({ success: true, data: [] });
});
app13.get("/vehicles", async (c) => {
  return c.json({ success: true, data: [] });
});
app13.get("/employees", async (c) => {
  return c.json({ success: true, data: [] });
});
app13.get("/bays", async (c) => {
  return c.json({ success: true, data: [] });
});
app13.get("/customers", async (c) => {
  return c.json({ success: true, data: [] });
});
app13.get("/busy-times", async (c) => {
  return c.json({ success: true, data: [] });
});
app13.get("/export", async (c) => {
  return c.json({ success: true, data: "CSV DATA" });
});
var reports_default = app13;

// worker/routes/audit.ts
var app14 = new Hono2();
app14.get("/", requireRole("admin"), async (c) => {
  const { results } = await c.env.DB.prepare("SELECT * FROM audit_logs ORDER BY created_at DESC LIMIT 100").all();
  return c.json({ success: true, data: results });
});
var audit_default = app14;

// worker/routes/backup.ts
var app15 = new Hono2();
app15.get("/export", requireRole("admin"), async (c) => {
  return c.json({ success: true, data: { message: "Exporting data..." } });
});
app15.post("/import", requireRole("admin"), async (c) => {
  return c.json({ success: true });
});
app15.post("/clear-demo", requireRole("admin"), async (c) => {
  return c.json({ success: true });
});
var backup_default = app15;

// worker/routes/content.ts
var app16 = new Hono2();
app16.get("/", async (c) => {
  const { results } = await c.env.DB.prepare("SELECT * FROM content_blocks").all();
  return c.json({ success: true, data: results });
});
app16.put("/", requireRole("admin"), async (c) => {
...
app17.put("/", requireRole("admin"), async (c) => {
  const settings = await c.req.json();
  for (const [k, v] of Object.entries(settings)) {
    await c.env.DB.prepare("UPDATE settings SET setting_value = ? WHERE setting_key = ?").bind(v, k).run();
  }
  return c.json({ success: true });
});
var settings_default = app17;

// worker/index.ts
var app18 = new Hono2();
app18.use("*", cors({ origin: /* @__PURE__ */ __name(() => "*", "origin") }));
...
app18.route("/api/auth", auth_default);
app18.route("/api/services", services_default);
app18.route("/api/customers", customers_default);
app18.route("/api/vehicles", vehicles_default);
app18.route("/api/appointments", appointments_default);
app18.route("/api/bays", bays_default);
app18.route("/api/employees", employees_default);
app18.route("/api/payments", payments_default);
app18.route("/api/expenses", expenses_default);
app18.route("/api/gallery", gallery_default);
app18.route("/api/testimonials", testimonials_default);
app18.route("/api/reports", reports_default);
app18.route("/api/audit", audit_default);
app18.route("/api/backup", backup_default);
app18.route("/api/content", content_default);
app18.route("/api/settings", settings_default);
app18.route("/api/public", public_default);
app18.onError((err, c) => {
  console.error(`${err}`);
...
// .wrangler/tmp/bundle-YEjyzF/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-YEjyzF/middleware-loader.entry.ts
