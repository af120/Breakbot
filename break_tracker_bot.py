...
# --- DATABASE FUNCTIONS (Now fully Async) ---
...
async def db_start_break(user_id: int, full_name: str, username: Optional[str]) -> bool:
    now_iso = datetime.now(timezone.utc).isoformat()
    try:
        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute(
                "INSERT INTO active_breaks (user_id, full_name, username, start_time) VALUES (?, ?, ?, ?)",
...
async def db_end_break(user_id: int) -> Optional[dict]:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute("SELECT * FROM active_breaks WHERE user_id = ?", (user_id,)) as cursor:
        ...
        await db.execute(
            "INSERT INTO break_history (user_id, full_name, start_time, end_time, duration_mins) VALUES (?, ?, ?, ?, ?)",
            (user_id, row["full_name"], row["start_time"], end_dt.isoformat(), duration_mins),
        )
        await db.execute("DELETE FROM active_breaks WHERE user_id = ?", (user_id,))
        await db.commit()
...
async def db_get_active_breaks():
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute("SELECT * FROM active_breaks") as cursor:
            return await cursor.fetchall()

async def db_mark_alerted(user_id: int) -> None:
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("UPDATE active_breaks SET alerted = 1 WHERE user_id = ?", (user_id,))
        await db.commit()
...
async def cb_take_break(callback: CallbackQuery) -> None:
    ...
    success = await db_start_break(user.id, full_name, username)

    if not success:
        await callback.answer("⚠️ أنت بالفعل في استراحة! اضغط 'تم الانطلاق' عند عودتك.", show_alert=True)
        return
    ...
    try:
        ...
        logger.error(f"Failed to send break start message: {e}")

@dp.callback_query(F.data == "back_to_work")
async def cb_back_to_work(callback: CallbackQuery) -> None:
    user = callback.from_user
    result = await db_end_break(user.id)

    if result is None:
        await callback.answer("⚠️ لم تسجّل استراحة! اضغط 'أخذ استراحة' أولاً.", show_alert=True)
        return
    ...
    status_line = "⚠️ <b>تجاوز الحد المسموح به!</b>" if over_limit else "✅ ضمن الوقت المسموح به."
...
async def cb_show_resting(callback: CallbackQuery) -> None:
    ...
    for row in rows:
        ...
        lines.append(f"• {user_mention(row['full_name'], row['user_id'])} — {elapsed_str}{warning}")

    try:
        await callback.message.answer("\n".join(lines))
    except TelegramAPIError as e:
        logger.error(f"Failed to send active breaks list: {e}")
        ...
        for row in rows:
            ...
            if elapsed_mins >= BREAK_LIMIT_MINUTES:
                ...
                try:
                    ...
                    await db_mark_alerted(row["user_id"])
                except TelegramAPIError as e:
                    logger.error(f"Failed to send overdue alert for user {row['user_id']}: {e}")
